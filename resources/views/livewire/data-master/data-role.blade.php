<div>
    @include('modal.data-master.data-role-modal')
    <x-slot name="header">
        Role User
            ...
            <x-table>
                <x-table.thead>
                    <tr class="table-primary">
                        <x-table.th class="text-center">
                            @if($firstId != NULL && !empty($roles) && isset($roles[0]))
                            <input class="form-check-input" type="checkbox" value="" id="flexCheckDefault"
                                wire:model.live='selectAll'>
                            <input type="text" hidden wire:model.live='firstId' value="{{$roles[0]->id}}">
                            @endif
                        </x-table.th>
                        <x-table.th>
                            Nama
                ...
                <x-table.tbody>
                    ...
                    <tr>
                        ...
                        <x-table.td>{{$r->created_at->format('d M Y')}}</x-table.td>
                        <x-table.td class="text-center">
                            <x-button.index wire:click='ubah({{$r->id}})' class="btn-success btn-sm">
                                Ubah
                            </x-button.index>
                            <x-button.index wire:click='hapus({{$r->id}})' class="btn-danger btn-sm">
                                Hapus
                            </x-button.index>
                        </x-table.td>
                    </tr>
